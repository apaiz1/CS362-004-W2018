...
int main() {
	...
	int seed = 1000;
	printf("Beginning test of village card.\n");
	for (i = 0; i < numRuns; i++) {
		...
		villagePlay(&test1, activePlayer);

		//tests
		if (test1.handCount[activePlayer] == original.handCount[activePlayer]) {
			printf("Passed - Player %d hand count is %d and should be %d.\n", activePlayer, test1.handCount[activePlayer], original.handCount[activePlayer]);
		}
		else {
		...
		*/
		if (test1.deckCount[activePlayer] == original.deckCount[activePlayer] - 1) {
			printf("Passed - Player %d deck count is %d and should be %d.\n", activePlayer, test1.deckCount[activePlayer], original.deckCount[activePlayer] - 1);
		}
		else {
	...
	printf("%d total failures over %d tests of village.\n", failure, numRuns);
