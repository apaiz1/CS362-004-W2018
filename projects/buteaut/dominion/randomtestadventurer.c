...
** Program: Dominion - Card Test 2 (adventurer)
** Class: CS362-400
** Date: 2-18-18
** Description: This is a card test for the adventurer card in dominion.c
***************************************************************************/
...
int main() {
	...
	for (i = 0; i < numRuns; i++) {
		...
		if (test1.handCount[activePlayer] == original.handCount[activePlayer] + 2) {
			printf("Passed - Player %d hand count is %d and should be %d.\n", activePlayer, test1.handCount[activePlayer], original.handCount[activePlayer] + 1);
		}
		else {
		...
		*/
		if (test1.deckCount[activePlayer] <= original.deckCount[activePlayer] - 2) {
			printf("Passed - Player %d deck count is %d and should be less than or equal to %d.\n", activePlayer, test1.deckCount[activePlayer], original.deckCount[activePlayer] - 2);
		}
		else {
			printf("Failed - Player %d deck count is %d and should be less than or equal to %d.\n", activePlayer, test1.deckCount[activePlayer], original.deckCount[activePlayer] - 2);
			failure++;
			failureLoop++;
		}
		/*
	...
	printf("%d total failures over %d tests of adventurer.\n", failure, numRuns);
