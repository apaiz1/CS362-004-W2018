...
void playAdventurer( struct gameState * state, int currentPlayer, int * z, int * cardDrawn, int * drawntreasure, int * temphand)
{
  while(*drawntreasure <2){
    ...
    *cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer]-1];//top card of hand is most recently drawn card.
    ...
    //drawntreasure should be 1 or 2
...
void playGreatHall( struct gameState * state, int currentPlayer, int handPos)
{
      ...
      // changed back to one after it was reviewed in my debugger
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      playAdventurer(state, currentPlayer, &z, & cardDrawn, & drawntreasure, temphand);
      return 0;
